...
import type { TestResult, RuleVariable } from '@/lib/platform-rule-api';
...
export function RuleTestPanel({
  ...
  const getDefaultValue = (name: string): number => {
    if (name === 'value') {
      if (type === 'pricing') return 100;
      if (type === 'settlement_fee') return 1000;
      return 50;
    }
    if (name === 'merchantPrice') return 100;
    if (name === 'cost') return 80;
    if (name === 'orderAmount') return 1000;
    return 0;
  };
