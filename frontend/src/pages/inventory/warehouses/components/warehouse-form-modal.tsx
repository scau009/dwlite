...
import {
  ...
  type CreateMerchantWarehouseParams,
} from '@/lib/merchant-warehouse-api';

interface WarehouseFormModalProps {
  open: boolean;
  warehouse: MerchantWarehouse | null;
  onClose: () => void;
  onSuccess: () => void;
}
...
export function WarehouseFormModal({
  ...
  const handleSubmit = async () => {
    try {
      ...
      const data: CreateMerchantWarehouseParams = {
        ...values,
      };

      if (isEdit) {
        await merchantWarehouseApi.updateWarehouse(warehouse.id, data);
        message.success(t('merchantWarehouses.updated'));
      } else {
        await merchantWarehouseApi.createWarehouse(data);
        message.success(t('merchantWarehouses.created'));
      }
  ...
  const tabItems = [
      ...
      children: (
        <Row gutter={16}>
          <Col span={12}>
            ...
            >
              <Input placeholder="WH-001" disabled={isEdit} />
            </Form.Item>
          </Col>
          <Col span={12}>
            <Form.Item
    ...
    },
    {
      ...
      children: (
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item name="contactName" label={t('warehouses.contactName')}>
              <Input placeholder={t('warehouses.contactNamePlaceholder')} />
            </Form.Item>
          </Col>
          <Col span={12}>
            <Form.Item name="contactPhone" label={t('warehouses.contactPhone')}>
  ...
  return (
    <Modal
      title={isEdit ? t('merchantWarehouses.edit') : t('merchantWarehouses.create')}
      open={open}
      onCancel={onClose}
      onOk={handleSubmit}
