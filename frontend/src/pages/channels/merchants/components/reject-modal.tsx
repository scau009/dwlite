...
export function RejectModal({ open, merchantChannel, onClose, onSuccess }: RejectModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await channelApi.rejectChannel(merchantChannel.id, values.reason);
      message.success(t('merchantChannels.rejected'));
      form.resetFields();
      onSuccess();
    } catch (error) {
  ...
  return (
    <Modal
      title={t('merchantChannels.rejectTitle')}
      open={open}
    ...
    >
      ...
      >
        <Form.Item
          name="reason"
          label={t('merchantChannels.rejectReason')}
          rules={[
            { required: true, message: t('merchantChannels.rejectReasonRequired') },
            { max: 500, message: t('merchantChannels.reasonMaxLength') },
          ]}
        >
          <Input.TextArea
            rows={3}
            placeholder={t('merchantChannels.rejectReasonPlaceholder')}
          />
        </Form.Item>
      </Form>
    </Modal>
